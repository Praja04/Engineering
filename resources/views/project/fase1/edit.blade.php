            ...
            <div class="col-xl-8 col-lg-10">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form method="POST" action="{{ route('project.fase1.update', $project) }}"
                              enctype="multipart/form-data" id="formFase1Edit">
                            @csrf @method('PUT')
                            ...
                            <div class="row gy-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="nomor_moc"
                                           class="form-control @error('nomor_moc') is-invalid @enderror"
                                           value="{{ old('nomor_moc', $project->nomor_moc) }}">
                                ...
                                <div class="col-12">
                                    ...
                                    <input type="text" name="deskripsi"
                                           ...
                                           value="{{ old('deskripsi', $faseSatu?->deskripsi ?? $project->deskripsi) }}">
                                    ...
                                    <select name="user_id" id="user_id"
                                        ...
                                        @foreach ($users as $user)
                                            <option value="{{ $user->id }}"
                                                {{ old('user_id', $faseSatu?->user_id ?? $project->user_id) == $user->id ? 'selected' : '' }}>
                                                {{ $user->username }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea name="keterangan" rows="3"
                                              ...
                                              placeholder="Catatan tambahan (opsional)">{{ old('keterangan', $faseSatu?->keterangan ?? $project->keterangan) }}</textarea>
                                            ...
                                            @foreach ($dokumen as $dok)
                                                <div class="position-relative border rounded p-1" style="min-width:80px">
                                                    @if ($dok->tipe === 'foto')
                                                        <a href="{{ Storage::url($dok->path_file) }}" target="_blank">
                                                            <img src="{{ Storage::url($dok->path_file) }}"
                                                                 class="doc-thumb d-block">
                                                        </a>
                                                    @else
                                                        <a href="{{ Storage::url($dok->path_file) }}" target="_blank"
                                                           class="d-flex flex-column align-items-center text-muted text-decoration-none p-2">
                                                    ...
                                                    <form method="POST"
                                                          ...
                                                          class="form-hapus-dok">
                                                        @csrf @method('DELETE')
                                                        <button type="submit"
                                                                class="btn btn-danger btn-xs position-absolute top-0 end-0"
                                                                style="width:18px;height:18px;padding:0;font-size:.6rem">
                                ...
                                <div class="col-12">
                                    ...
                                    <div class="upload-area" onclick="document.getElementById('dokumentasi').click()">
...
<script>
    ...
    function renderPreview() {
        ...
        Array.from(input.files).forEach(f => {
            ...
            preview.innerHTML += `<div class="file-item"><i class="${icon} text-primary"></i><span class="flex-grow-1 text-truncate">${f.name}</span><small class="text-muted">${(f.size/1024).toFixed(1)} KB</small></div>`;
        });
    }
    ...
    document.querySelectorAll('.form-hapus-dok').forEach(form => {
        form.querySelector('button').addEventListener('click', e => {
            e.preventDefault();
            Swal.fire({ title: 'Hapus file?', icon: 'warning', showCancelButton: true,
                confirmButtonText: 'Hapus', cancelButtonText: 'Batal', confirmButtonColor: '#f06548' })
