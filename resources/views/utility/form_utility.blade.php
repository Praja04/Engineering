...
@section('content')
                        ...
                        <div class="card-body">
                                ...
                                <form id="form-pemakaian-listrik">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="date" name="waktu" class="form-control" id="waktu_listrik"
                                            @if (Auth::user()->jabatan === 'operator') readonly @endif>
                                    </div>
                                    <div class="mb-3">
                            ...
                            <div id="form-air" style="display: none;">
                                <form id="form-pemakaian-air">
                                    @csrf
                                    <div class="mb-3">
                                        <label for="tanggal" class="form-label">Tanggal (Pemakaian Tanggal
                                            Sebelumnya)</label>
                                        <input type="date" name="tanggal" class="form-control" id="tanggal_air"
                                            @if (Auth::user()->jabatan === 'operator') readonly @endif>
                                    </div>
    ...
    <script>
        ...
        function setTanggalChemical() {
            ...
            const isOperator = inputTanggal.hasAttribute('readonly');

            if (selectedShift === 'shift 3') {
                // sebelum jam 08:00 → tanggal mundur
                if (totalDetik < batasAwalShift3) {
                    tanggal.setDate(tanggal.getDate() - 1);
                }
                // shift 3 → semua boleh edit
                inputTanggal.removeAttribute('readonly');
            } else {
                // selain shift 3 → hanya operator yang readonly
                if (isOperator) {
        ...
        $(document).ready(function() {
            ...
            $.get("{{ url('utility/chemical-area') }}", function(data) {
        ...
        `;
            ...
            $('#chemical-area').change(function() {
                ...
                if (areaId) {
                    $.ajax({
                        url: "{{ url('utility/chemical-types') }}/" + areaId,
                        type: 'GET',
                        success: function(data) {
                            ...
                            if (data.length === 0) {
                                $('#chemical-input-container').html(
                                    '<p class="text-danger">Tidak ada chemical untuk area ini.</p>'
                                );
                                return;
                            }
                            ...
                            data.forEach((chemical) => {
                                ...
                                const isDefoamer = chemical.nama_chemical.toLowerCase()
                                    .includes('defoamer');
                        ...
                        `;
                                    ...
                                    runningHourInput =
                                        `<input type="hidden" name="running_hour[]" value="">`;
                                }
            ...
            $('#chemical-form').submit(function(e) {
                ...
                $.ajax({
                    url: "{{ url('utility/store/chemical') }}", // Ganti dengan route POST kamu
                    method: "POST",
                    data: formData,
                    headers: {
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#clearDraftListrik").on("click", function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        Swal.fire("Berhasil", "Draft untuk panel " + panel + " dihapus.",
                            "success");
                    }
                });
            });
            ...
            $("#form-pemakaian-listrik").on("submit", function(e) {
                ...
                $.ajax({
                    url: "{{ url('utility/data/listrik/store') }}",
                    method: "POST",
                    data: formData,
