    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header border-0">
                <h5 class="modal-title fw-bold">Detail Log AHU</h5>
                            ...
                            <div class="card-body p-3">
                                ...
                                <div class="row g-2">
                                    <div class="col-4">
                                        ...
                                        <input type="number" step="0.01" name="ampere_{{$i}}" id="edit_ampere_{{$i}}" class="form-control form-control-sm">
                                    </div>
                                    <div class="col-4">
                                        <label class="small mb-1">Set Temp</label>
                                        <input type="number" step="0.01" name="set_temp_{{$i}}" id="edit_set_temp_{{$i}}" class="form-control form-control-sm">
                                    </div>
                                    <div class="col-4">
                                        <label class="small mb-1">Press In</label>
                                        <input type="number" step="0.01" name="pressure_in_{{$i}}" id="edit_pressure_in_{{$i}}" class="form-control form-control-sm">
                                    </div>
                                    <div class="col-4">
                                        <label class="small mb-1">Press Out</label>
                                        <input type="number" step="0.01" name="pressure_out_{{$i}}" id="edit_pressure_out_{{$i}}" class="form-control form-control-sm">
                                    </div>
                                    <div class="col-4">
                                        <label class="small mb-1">CT In</label>
                                        <input type="number" step="0.01" name="ct_in_{{$i}}" id="edit_ct_in_{{$i}}" class="form-control form-control-sm">
                                    </div>
                                    <div class="col-4">
                                        <label class="small mb-1">CT Out</label>
                                        <input type="number" step="0.01" name="ct_out_{{$i}}" id="edit_ct_out_{{$i}}" class="form-control form-control-sm">
                                    </div>
                                    <div class="col-12">
                                        <label class="small mb-1">Temp Out</label>
                                        <input type="number" step="0.01" name="temp_out_{{$i}}" id="edit_temp_out_{{$i}}" class="form-control form-control-sm">
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                res.data.forEach(item => {
                    let st = '';
                    if (item.approval_status == 'draft') st = '<span class="badge bg-warning">Draft</span>';
                    else if (item.approval_status == 'submitted') st = '<span class="badge bg-info">Submitted</span>';
                    else if (item.approval_status == 'approved_foreman') st = '<span class="badge bg-primary">Approve FM</span>';
                    else if (item.approval_status == 'approved_supervisor') st = '<span class="badge bg-success">Approved</span>';
                    else if (item.approval_status == 'rejected') st = '<span class="badge bg-danger">Rejected</span>';
                ...
                $('#tbodyData').html(html || '<tr><td colspan="8" class="text-center">Tidak ada data</td></tr>');
    ...
    function showDetail(id) {
        $.get("{{ url('utility/ahu/show') }}/" + id, function(res) {
            ...
            [1, 2, 3, 4].forEach(i => {
                html += `
                    <div class="col-md-6">
                        <div class="p-3 border rounded ahu-detail-card">
                            <h6 class="fw-bold">AHU ${i}</h6>
                            <small class="text-muted">Ampere: ${fN(d['ampere_'+i])}</small><br>
                            <small class="text-muted">Set Temp: ${fN(d['set_temp_'+i])}°C</small><br>
                            <small class="text-muted">Press In: ${fN(d['pressure_in_'+i])} Bar</small><br>
                            <small class="text-muted">Press Out: ${fN(d['pressure_out_'+i])} Bar</small><br>
                            <small class="text-muted">CT In: ${fN(d['ct_in_'+i])} A</small><br>
                            <small class="text-muted">CT Out: ${fN(d['ct_out_'+i])} A</small><br>
                            <small class="text-muted">Temp Out: ${fN(d['temp_out_'+i])}°C</small>
                        </div>
                    </div>
                `;
            });
