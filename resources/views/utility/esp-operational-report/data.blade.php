                    ...
                    <div class="card border-0 shadow-sm">
                        <div class="card-header bg-transparent border-bottom">
                            <h6 class="mb-0 fw-semibold">
                                ...
                                <span class="badge bg-primary-subtle text-primary ms-1" id="badge-count-operational">—</span>
                            </h6>
                        </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover table-bordered mb-0 text-center align-middle">
                                    ...
                                    <tbody id="tbody-operational">
                                        <tr><td colspan="8" class="py-4 text-muted">Memuat data...</td></tr>
                                    </tbody>
                                </table>
                            </div>
                        </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover table-bordered mb-0 align-middle">
                                    ...
                                    <tbody id="tbody-shift">
                                        <tr><td colspan="9" class="py-4 text-muted text-center">Memuat data...</td></tr>
                                    </tbody>
                                </table>
                            </div>
                        </div>
        ...
        <div class="modal-content border-0 shadow">
            ...
            <div class="modal-body">
                <table class="table table-bordered table-sm mb-0">
                    <tbody>
                        <tr><th class="w-50 text-muted">Jam</th><td id="d-op-jam">—</td></tr>
                        <tr><th class="text-muted">Grup</th><td id="d-op-grup">—</td></tr>
                        <tr><th class="text-muted">Arus Primer</th><td id="d-op-arus-primer">—</td></tr>
                        <tr><th class="text-muted">Arus Sekunder</th><td id="d-op-arus-sekunder">—</td></tr>
                        <tr><th class="text-muted">Tegangan Primer</th><td id="d-op-teg-primer">—</td></tr>
                        <tr><th class="text-muted">Tegangan Sekunder</th><td id="d-op-teg-sekunder">—</td></tr>
                        <tr><th class="text-muted">Suhu Thermal</th><td id="d-op-suhu">—</td></tr>
                    </tbody>
                </table>
            </div>
        ...
        <div class="modal-content border-0 shadow">
            <div class="modal-header border-bottom">
                <h5 class="modal-title fw-semibold">
                    <i class="ri-edit-line me-2 text-primary"></i>Edit Operational — <span id="edit-op-jam-label"></span>
                </h5>
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        ...
                        <div class="col-6">
                            ...
                            <input type="number" step="0.01" class="form-control" id="edit-op-arus-sekunder">
                        </div>
                        <div class="col-6">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <table class="table table-sm table-bordered mb-0">
                            <tr><th class="text-muted">Air</th><td id="d-sh-air">—</td></tr>
                            <tr><th class="text-muted">Steam</th><td id="d-sh-steam">—</td></tr>
                            <tr><th class="text-muted">Batubara</th><td id="d-sh-batubara">—</td></tr>
                            <tr><th class="text-muted">Efisiensi Batubara</th><td id="d-sh-efisiensi">—</td></tr>
                            <tr><th class="text-muted">Pengisian Batubara</th><td id="d-sh-pengisian">—</td></tr>
                        </table>
                    </div>
                    <div class="col-md-6">
                        <h6 class="fw-semibold text-muted border-bottom pb-2">Running Hour & Feed Tank</h6>
                        <table class="table table-sm table-bordered mb-0">
                            <tr><th class="text-muted">RH Awal</th><td id="d-sh-rh-awal">—</td></tr>
                            <tr><th class="text-muted">RH Akhir</th><td id="d-sh-rh-akhir">—</td></tr>
                            <tr><th class="text-muted">Feed Tank Awal</th><td id="d-sh-ft-awal">—</td></tr>
                            <tr><th class="text-muted">Feed Tank Akhir</th><td id="d-sh-ft-akhir">—</td></tr>
                        </table>
                        <h6 class="fw-semibold text-muted border-bottom pb-2 mt-3">Chemical</h6>
                        <table class="table table-sm table-bordered mb-0">
                            <tr><th class="text-muted">Chemical SCF</th><td id="d-sh-scf">—</td></tr>
                            <tr><th class="text-muted">Chemical SRTF</th><td id="d-sh-srtf">—</td></tr>
                            <tr><th class="text-muted">Dosis</th><td id="d-sh-dosis">—</td></tr>
                        </table>
                    </div>
                    <div class="col-12">
...
<div class="modal fade" id="modal-edit-shift" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content border-0 shadow">
            <div class="modal-header border-bottom">
                <h5 class="modal-title fw-semibold">
                    <i class="ri-edit-line me-2 text-danger"></i>Edit Shift Report — <span id="edit-sh-tgl-label"></span>
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
...
$(function () {
    ...
    $('#filter-tanggal, #filter-grup').on('change', function () {
        activeTab === 'operational' ? loadOperational() : loadShift();
    });
    ...
    function loadOperational() {
        ...
        $.get('{{ route("esp-operational-report.json") }}', { tanggal, grup }, function (data) {
    ...
    // ── LOAD SHIFT ───────────────────────────────────────────────────
    const statusMap = {
        'approved_operator'  : { label: 'Menunggu Foreman',    cls: 'bg-warning text-dark' },
        'approved_foreman'   : { label: 'Menunggu Supervisor', cls: 'bg-info text-dark'    },
        'approved_supervisor': { label: 'Final Approved',      cls: 'bg-success text-white'},
    };
    ...
    function loadShift() {
        ...
        $.get('{{ route("esp-shift-report.json") }}', { tanggal }, function (data) {
            if (!data.length) {
                $('#tbody-shift').html(
            ...
            let html = '';
            data.forEach(function (r) {
                const s = statusMap[r.status] ?? { label: r.status, cls: 'bg-secondary text-white' };
    ...
    $('#form-edit-operational').on('submit', function (e) {
        ...
        $.ajax({
            ...
            complete: function () {
                $btn.prop('disabled', false).html('<i class="ri-save-line me-1"></i>Simpan');
            }
        });
    });
    ...
    $(document).on('click', '.btn-detail-shift', function (e) {
        ...
        // Timeline approval
        const steps = [
            { key: 'approved_operator',   label: 'Operator',   icon: 'ri-user-line'        },
            { key: 'approved_foreman',    label: 'Foreman',    icon: 'ri-user-star-line'   },
            { key: 'approved_supervisor', label: 'Supervisor', icon: 'ri-shield-check-line'},
        ];
        ...
        steps.forEach(function (s, i) {
            ...
            tl += `<div class="d-flex flex-column align-items-center gap-1">
