...
<script>
    ...
    $(document).ready(function() {
        ...
        function loadData() {
            ...
            tbody.html('<tr><td colspan="3" class="text-center"><div class="spinner-border spinner-border-sm" role="status"></div> Memuat data...</td></tr>');
            ...
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    ...
                    tbody.html('<tr><td colspan="3" class="text-center text-danger">Gagal memuat data. Silakan coba lagi.</td></tr>');
                    Swal.fire({
                        title: 'Error!',
        ...
        $(document).on('click', '.view-detail', function() {
            ...
            $('#modalContent').html('<div class="text-center"><div class="spinner-border text-primary" role="status"></div><p class="mt-2">Loading data...</p></div>');
        ...
        function renderPagination(data, itemsPerPage = 10) {
            ...
            function renderTableRows(dataSlice) {
                ...
                if (!dataSlice.length) {
                    tbody.append('<tr><td colspan="3" class="text-center text-muted py-4"><i class="ri-file-list-line fs-1 d-block mb-2"></i>Tidak ada data ditemukan.</td></tr>');
                    return;
                }
            ...
            function buildButtons() {
                ...
                // Previous button
                if (currentPage > 1) {
                    const prevBtn = $(`<button class="btn btn-sm btn-outline-primary mx-1"><i class="ri-arrow-left-s-line"></i></button>`);
                    prevBtn.on('click', () => {
                        currentPage--;
                ...
                if (currentPage < totalPages) {
                    const nextBtn = $(`<button class="btn btn-sm btn-outline-primary mx-1"><i class="ri-arrow-right-s-line"></i></button>`);
                    nextBtn.on('click', () => {
                        currentPage++;
                        renderPage(currentPage);
                        buildButtons();
        ...
        function buildChemicalTable(entry) {
            ...
            const canEdit = userJabatan !== 'operator';
        ...
        $('#editPanelForm').on('submit', function(e) {
            ...
            $.post(url, formData, function(res) {
                ...
                const errorMsg = xhr.responseJSON?.message || 'Gagal menyimpan data. Silakan periksa kembali.';
                Swal.fire({
                    title: 'Gagal!',
                    text: errorMsg,
                    icon: 'error'
