        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                res.data.forEach(item => {
                    let statusBadge = '';
                    if (item.status == 'submitted') statusBadge = '<span class="badge bg-info">Menunggu Foreman</span>';
                    else if (item.status == 'approved_foreman') statusBadge = '<span class="badge bg-primary">Menunggu Supervisor</span>';
                    else if (item.status == 'approved_supervisor') statusBadge = '<span class="badge bg-success">Approved</span>';
                    else if (item.status == 'rejected') statusBadge = `<span class="badge bg-danger" title="${item.reject_reason || ''}">Rejected</span>`;
                ...
                if (res.data.length == 0) {
                    html = '<tr><td colspan="9" class="text-center">Tidak ada data menunggu approval</td></tr>';
                }
