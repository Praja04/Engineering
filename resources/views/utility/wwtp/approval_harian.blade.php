        ...
        $(document).ready(function() {
            ...
            $('#btnMassApprove').on('click', function() {
                ...
                Swal.fire({
                    ...
                    text: `Apakah Anda yakin ingin menyetujui ${selectedIds.length} data harian WWTP terpilih?`,
                    icon: 'question',
                    showCancelButton: true,
                    confirmButtonColor: '#28a745',
                    ...
                    if (result.isConfirmed) {
                        showLoading(true);
                        $.ajax({
                            url: "{{ url('wwtp-approval/mass-approve') }}",
                            method: 'POST',
                            data: {
                                _token: csrfToken,
                                ids: selectedIds
            ...
            $('#btnMassReject').on('click', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed && result.value) {
                        showLoading(true);
                        $.ajax({
                            url: "{{ url('wwtp-approval/mass-reject') }}",
                            method: 'POST',
                            data: {
                                _token: csrfToken,
                                ids: selectedIds,
