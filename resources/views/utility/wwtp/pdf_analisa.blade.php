...
<body>
    ...
    <table class="meta-table">
        <tr>
            ...
            <td class="meta-value">: {{ \Carbon\Carbon::parse($analisa->analisa_date)->locale('id')->translatedFormat('d F Y') }}</td>
                    ...
                    @foreach ($param['points'] as $point)
                        @php
                            ...
                            if ($stdVal !== null) {
                                ...
                                $stdDisplay = ($parsedStd == (int) $parsedStd ? number_format($parsedStd, 0, ',', '.') : number_format($parsedStd, 2, ',', '.')) . ($param['unit'] ? ' ' . $param['unit'] : '');
                            }
                            ...
                            if ($value !== null) {
                                ...
                                $valDisplay = ($parsedVal == (int) $parsedVal ? number_format($parsedVal, 0, ',', '.') : number_format($parsedVal, 2, ',', '.')) . ($param['unit'] ? ' ' . $param['unit'] : '');
                            }
                        @endphp
                        <tr>
    ...
    <!-- Footer -->
    <div class="footer">
        PT. BAS &copy; {{ date('Y') }} | Halaman <span class="page-number"></span> dari Laporan Analisa WWTP Tanggal {{ \Carbon\Carbon::parse($analisa->analisa_date)->format('d-m-Y') }}
    </div>
</body>
</html>
