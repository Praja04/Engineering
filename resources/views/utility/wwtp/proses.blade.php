        ...
        <div class="modal-content">
            <div class="modal-header bg-light">
                <h5 class="modal-title" id="modalFormLabel">
                    <i class="mdi mdi-file-document-edit me-2"></i>Tambah Data WWTP
                </h5>
                ...
                columns: [{
                                ...
                                return `
                                    <div class="d-flex flex-column gap-1">
                                        <div class="d-flex justify-content-between">
                                            ...
                                            <span class="fw-semibold">${parseFloat(data.influent.pit_sparta).toFixed(2)} m³</span>
                                        </div>
                                        <div class="d-flex justify-content-between">
                                            <span class="text-muted small">Pit Garam:</span>
                                            <span class="fw-semibold">${parseFloat(data.influent.pit_garam).toFixed(2)} m³</span>
                                        </div>
                                        <div class="d-flex justify-content-between">
                                            <span class="text-muted small">Pit Domestik:</span>
                                            <span class="fw-semibold">${parseFloat(data.influent.pit_domestik).toFixed(2)} m³</span>
                                        </div>
                                    </div>
                                `;
                                ...
                                return `
                                    <div class="d-flex flex-column gap-1">
                                        <div class="d-flex justify-content-between">
                                            ...
                                            <span class="fw-semibold">${parseFloat(data.effluent.full_proses).toFixed(2)} m³</span>
                                        </div>
                                        <div class="d-flex justify-content-between">
                                            <span class="text-muted small">DAF Pre:</span>
                                            <span class="fw-semibold">${parseFloat(data.effluent.daf_pre).toFixed(2)} m³</span>
                                        </div>
                                    </div>
                                `;
                            }
                    ...
                    },
                    {
                        ...
                        render: function(data) {
                            let total = 0;
                            if (data.kategori === 'influent' && data.influent) {
                                total = parseFloat(data.influent.pit_sparta) +
                                    parseFloat(data.influent.pit_garam) +
                                    parseFloat(data.influent.pit_domestik);
                            } else if (data.kategori === 'effluent' && data.effluent) {
                                total = parseFloat(data.effluent.full_proses) +
        ...
        $('#filterMonth').on('change', function() {
            ...
            if (value) {
                const monthFilter = function(settings, data, dataIndex) {
                    ...
                    const date = new Date(rowData.tanggal + 'T00:00:00'); // Add time to ensure consistent parsing
                    const [selectedYear, selectedMonth] = value.split('-');

                    return date.getMonth() === (parseInt(selectedMonth) - 1) &&
                        date.getFullYear() === parseInt(selectedYear);
                };
                monthFilter.name = 'monthFilter'; // Add name for identification
