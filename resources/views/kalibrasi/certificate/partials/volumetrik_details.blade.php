                     ...
                     <div class="table-responsive">
                         <table class="table table-hover table-sm text-center mb-0">
                             ...
                             <tbody id="hitung_data">
                                 @foreach ($main->volumetrik as $volumetrik)
                                     @php
                                         $detailCount = $volumetrik->details->count();
                                     @endphp
                                     @foreach ($volumetrik->details as $index => $detail)
                                         <tr>
                                             @if ($index == 0)
                                                 <td rowspan="{{ $detailCount }}" class="align-middle fw-bold">
                                                     {{ formNum($volumetrik->titik_kalibrasi) }}
                                                 </td>
                                             @endif
                                             <td>{{ formNum($detail->penunjuk_standar) }}</td>
                                             <td>{{ formNum($detail->penunjuk_alat) }}</td>
                                             <td>{{ formNum($detail->koreksi) }}</td>
                                         </tr>
                                     @endforeach
                                 @endforeach
                             </tbody>
                     ...
                     <div class="table-responsive">
                         <table class="table table-hover table-sm text-center mb-0">
                             ...
                             <tbody id="detail_gabungan">
                                 @foreach ($main->volumetrik as $volumetrik)
                                     <tr>
                                         <td>{{ formNum($volumetrik->titik_kalibrasi) }}</td>
                                         <td>{{ formNum($volumetrik->avg_penunjuk_standar) }}</td>
                                         <td>{{ formNum($volumetrik->avg_koreksi) }}</td>
                                         <td>{{ formNum($volumetrik->stdev_penunjuk_standar) }}</td>
                                         <td>{{ formNum($volumetrik->akar_10) }}</td>
                                         <td>{{ formNum($volumetrik->u_timbangan) }}</td>
                                         <td>{{ formNum($volumetrik->u_total) }}</td>
                                     </tr>
                                 @endforeach
                             </tbody>
                         </table>
