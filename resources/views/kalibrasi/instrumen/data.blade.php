                ...
                <div class="col-xl-12">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">History Kalibrasi Instrumen</h4>
                        </div>
                        <div class="card-body">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body p-0">
                    ...
                    <ul class="nav nav-pills arrow-navtabs nav-success bg-light mb-3 px-3 pt-3" role="tablist">
                        ...
                        <li class="nav-item">
                            <button class="nav-link" data-bs-toggle="tab"
                                data-bs-target="#pengukuran-detail-pane">Pengukuran Detail</button>
                        </li>
    ...
    <script>
        $(document).ready(function() {
            ...
            function fetchHistoryData() {
                $.ajax({
                    url: `{{ url('api/kalibrasi/instrumen/data') }}`,
                    method: 'GET',
                    dataType: 'json',
            ...
            function showDetailModal(id, historyData) {
                ...
                if (item.keypad && item.keypad.length > 0) {

                    item.keypad.forEach(kp => {
                        keypadHtml += `
                            <tr>
                                <td>
                                    ${kp.tested ? 
                    ...
                    keypadHtml = `
                        <tr>
                            <td colspan="4" class="text-muted text-center">
                                Tidak ada data keypad
                            </td>
                        </tr>
                    `;
                }

                $('#keypad').html(keypadHtml);

                let utamaHtml = '';

                if (item.instrumen && item.instrumen.length > 0) {

                    item.instrumen.forEach(ins => {
                        ...
                        utamaHtml += `
                            <tr>
                                ...
                                <td>${formatNumber(ins.nilai_master, 2)}</td>
                                <td>${formatNumber(ins.avg_pembacaan, 2)}</td>
                                <td>${formatNumber(ins.std_dev, 2)}</td>
                                <td>${formatNumber(ins.koreksi, 2)}</td>
                            </tr>
                        `;
                    });
                ...
                if (item.instrumen && item.instrumen.length > 0) {
                    ...
                    item.instrumen.forEach(ins => {
                        ...
                        if (ins.details && ins.details.length > 0) {
                            ...
                            let totalDetail = ins.details.length;

                            ins.details.forEach((det, index) => {

                                detailHtml += `<tr>`;

                                // ✅ Hanya tampil sekali dengan rowspan
                                if (index === 0) {
                                    detailHtml += `
                                        <td rowspan="${totalDetail}" class="fw-bold align-middle">
                                            ${ins.titik_kalibrasi}
                                        </td>
                                    `;
                                }
                                ...
                                detailHtml += `
                                    <td>${formatNumber(det.alat, 2)}</td>
                                    <td>${formatNumber(det.standar, 2)}</td>
                                    <td>${formatNumber(det.pembacaan_alat, 2)}</td>
                                    <td>${formatNumber(det.pembacaan_standar, 2)}</td>
                                </tr>`;
                            });
                    ...
                    detailHtml = `
                        <tr>
                            <td colspan="5" class="text-muted text-center">
                                Tidak ada detail pengukuran
                            </td>
                        </tr>
                    `;
                }

                $('#detail_data').html(detailHtml);
