                ...
                <div class="col-xl-12">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">History Kalibrasi Flow Meter</h4>
                        </div>
                        <div class="card-body">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body p-0">
                    ...
                    <ul class="nav nav-pills arrow-navtabs nav-success bg-light mb-3 px-3 pt-3" role="tablist">
                        ...
                        <li class="nav-item">
                            <button class="nav-link" data-bs-toggle="tab" data-bs-target="#result-pane">Summary</button>
                        </li>
                    </ul>
    ...
    <script>
        $(document).ready(function() {
            ...
            function fetchHistoryData() {
                $.ajax({
                    url: `{{ url('api/kalibrasi/flowmeter/data') }}`,
                    method: 'GET',
                    dataType: 'json',
            ...
            function showDetailModal(id, historyData) {
                ...
                const flowMeters = Array.isArray(item.flowmeter) ? item.flowmeter : [];
                const hitungBody = $('#hitung_data');
                hitungBody.empty();

                if (flowMeters.length > 0) {

                    flowMeters.forEach(v => {
                    ...
                    flowMeters.forEach(v => {
                        ...
                        gabunganBody.append(`
                            <tr>
                                ...
                                <td>${formatNumber(v.nilai_master, 8)}</td>
                                <td>${formatNumber(v.avg_pembacaan, 8)}</td>
                                <td>${formatNumber(v.koreksi, 8)}</td>
                                <td>${formatNumber(v.std_dev, 8)}</td>
                                <td>${formatNumber(v.ketidakpastian, 8)}</td>
                                </td>
                            </tr>
                        `);
            ...
            $('#historyTable').on('click', '.delete-btn', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: `{{ route('kalibrasi.dimensi.delete', '') }}/` + id,
                            method: 'DELETE',
                            headers: {
