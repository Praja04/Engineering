...
@section('title', ' Data Check Mtc Utility')
    ...
    <div class="container-fluid">
        ...
        <div class="card card-soft shadow-sm">
            <div class="card-header d-flex align-items-center justify-content-between">
                <div>
                    <h4 class="fw-bold">Data Maintenance Utility</h4>
                    <div class="small-muted">List inspeksi + detail hasil pengecekan</div>
                </div>
                <div class="d-flex gap-2">
                    <a href="{{ url('/mtc/form/utility/index') }}" class="btn btn-primary">
                        + Input Baru
                    </a>
                </div>
            </div>
            ...
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <table class="table table-hover align-middle" id="tabelUtility">
...
<div class="modal fade" id="modalEdit" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <form id="formEditUtility">
                @csrf
                <input type="hidden" id="editId" name="id">

                <div class="modal-header">
                    <div>
                        <div class="fw-bold">Edit Inspeksi Mtc Utility</div>
...
<script>
    $(document).ready(function() {
        const API_URL = "{{ url('api/mtc/utility/get-data') }}";
        const DELETE_URL = "{{ url('mtc/main/delete') }}";
        const UPDATE_URL = "{{ url('mtc/data/utility/update') }}";
            ...
            const section = (title, items) => {
                const cells = items.map(([key, label]) => `
                        <div class="item-cell">
                            ...
                            <div>${statusBadge(row.utility?.[key])}</div>
                        </div>
                    `).join('');
            ...
            return `
                    <div class="detail-meta row g-3 mb-2">
                       <div class="col-md-3">
                            ...
                            <div class="meta-value">${row.utility.mesin.nama_mesin ?? '-'}</div>
                        </div>
                        <div class="col-md-3">
                            <div class="meta-label">Lokasi</div>
                            <div class="meta-value">${row.utility.mesin.lokasi ?? '-'}</div>
                        </div>
                        <div class="col-md-3">
                        ...
                        <div class="col-md-3">
                            ...
                            <div class="meta-value">${fmtDate(row.tanggal_selesai)}</div>
                        </div>
                        ` : ''}
                        <div class="col-md-2">
                    ...
                    ${section('Cooling Tower', fields.cooling_tower)}
                    ${section('RO', fields.ro)}
                    ${section('Compressor', fields.compressor)}
                    ${section('Tank Farm', fields.tank_farm)}
                    ${section('Boiler', fields.boiler)}
                    ${section('WWTP', fields.wwtp)}

                    <div class="row g-3 mt-2">
                        <div class="col-md-6">
                            <div class="group-title">Keterangan</div>
                            <div>${row.utility?.keterangan ?? '-'}</div>
                        </div>
                        <div class="col-md-6">
                            <div class="group-title">Tindakan Korektif</div>
                            <div>${row.utility?.korektif ?? '-'}</div>
                        </div>
                        <div class="col-md-12">
    ...
    : '<div>-</div>'
        ...
        const dtUtility = $('#tabelUtility').DataTable({
            processing: true,
            serverSide: true,
            searching: false,
            pageLength: 10,
            ...
            columns: [{
                ...
                {
                    data: 'utility.mesin.nama_mesin',
                    orderable: false,
                    defaultContent: '-'
                },
                {
                    data: 'utility.mesin.lokasi',
                    orderable: false,
                    defaultContent: '-'
                },
                {
        ...
        $(document).on('click', '.btn-detail', function() {
            ...
            $('#detailTitle').text('Detail Inspeksi Mtc Utility');
            $('#detailSub').text(`${fmtDate(row.tanggal)} - ${row.waktu_mulai ?? '-'}`);
        ...
        function renderEditSection(title, items, row) {
            const cells = items.map(([key, label]) => {
                const state = valToState(row.utility?.[key]);
                return `
        ...
        function buildEditForm(row) {
            const html = `
                    ${renderEditSection('Cooling Tower', fields.cooling_tower, row)}
                    ${renderEditSection('RO', fields.ro, row)}
                    ${renderEditSection('Compressor', fields.compressor, row)}
                    ${renderEditSection('Tank Farm', fields.tank_farm, row)}
                    ${renderEditSection('Boiler', fields.boiler, row)}
                    ${renderEditSection('WWTP', fields.wwtp, row)}
                `;
        ...
        $(document).on('click', '.btn-edit', function() {
            ...
            $('#editKorektif').val(row.utility?.korektif ?? '');
            $('#editNamaMesin')
                .val(row.utility?.mesin_id)
                .trigger('change');
            ...
            const ketMap = parseKeteranganPairs(row.utility?.keterangan);
        ...
        $('#formEditUtility').on('submit', function(e) {
            e.preventDefault();
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    $('#tabelUtility').DataTable().ajax.reload(null, false);
                },
                error: function(xhr) {
                    Swal.fire({
                        icon: 'error',
        ...
        $(document).on('click', '.btn-delete', function() {
            ...
            Swal.fire({
                icon: 'warning',
                title: 'Hapus data?',
                text: `Data Mtc Utility akan dihapus permanen`,
