...
@section('title', ' Form Check Mtc Refrigerasi')
        ...
        <div class="container-fluid">
            <div class="card shadow border-0">
                <div class="card-header bg-primary text-white fw-bold">
                    Form Check Maintenance Refigerasi
                </div>

                <div class="card-body">

                    <form id="form-mtc-refrigerasi">
                        @csrf
                        ...
                        <h6 class="fw-bold text-primary mb-3">Unit Indoor</h6>

                        @foreach (array_chunk($unitIndoor, 2, true) as $row)
                        ...
                        <h6 class="fw-bold text-primary mb-3">Unit Outdoor</h6>

                        @foreach (array_chunk($unitOutdoor, 2, true) as $row)
                            <div class="row g-3 mb-3">
                                @foreach ($row as $field => $label)
                                    <div class="col-md-6 col-12">
                                        <div class="card shadow-sm item-card p-3 item-row" data-field="{{ $field }}">
                        ...
                        <h6 class="fw-bold text-primary mb-3">Jalur Distribusi</h6>

                        @foreach (array_chunk($jalurDistribusi, 2, true) as $row)
                            <div class="row g-3 mb-3">
                                @foreach ($row as $field => $label)
                                    <div class="col-md-6 col-12">
...
@section('scripts')
    <script>
        $(document).ready(function() {
            const STORAGE_KEY = 'form_mtc_refrigerasi_data';

            // Fungsi untuk menyimpan form ke localStorage
            function saveFormToLocalStorage() {
                const formData = {};
                $('#form-mtc-refrigerasi').serializeArray().forEach(item => {
            ...
            $('#form-mtc-refrigerasi').on('change input', 'input, select, textarea', function() {
            ...
            $('#form-mtc-refrigerasi').on('submit', function(e) {
                ...
                $.ajax({
                    url: "{{ route('mtc.refrigerasi.store') }}",
                    type: 'POST',
                    data: formData,
