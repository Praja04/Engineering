                    ...
                    <form id="form-mtc-electric-p2h" method="POST">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-3">
                                <label class="form-label">Hours Meter (Jam Operasional)<span
                                        class="text-danger">*</span></label> {{-- Forklift / ES / PM (Pallet Mover) --}}
                                <input type="number" name="hours_meter" class="form-control"
                        ...
                        @php
                            ...
                            $electricP2h = [
                                ...
                                'lift_chains' => [
                                    // forklift & ES (Electric Stacker/Steker)
                                    'label' => 'Lift Chains',
                                ...
                                @foreach ($row as $field => $item)
                                    <div class="col-md-6 col-12">
                                        <div class="card shadow-sm item-card p-3 item-row" data-field="{{ $field }}"
                                            data-type="{{ $fieldTypes[$field] ?? 'all' }}">
    ...
    <script>
        $(document).ready(function() {
            ...
            function filterChecklistFields(namaMesin) {
                ...
                // Show/hide hours_meter input field
                const $hoursMeterCol = $('input[name="hours_meter"]').closest('.col-md-3');
                if (activeType === 'forklift' || activeType === 'pm' || activeType === 'es') {
                    $hoursMeterCol.removeClass('d-none');
                    $('input[name="hours_meter"]').attr('required', true);
                } else {
                    $hoursMeterCol.addClass('d-none');
                    $('input[name="hours_meter"]').val('').removeAttr('required');
                }

                $('.item-row').each(function() {
                    const $row = $(this);
                    const type = $row.data('type'); // 'forklift', 'all', 'forklift_es'
                    const $col = $row.closest('.col-md-6');

                    let visible = false;
                    if (!activeType) {
                        visible = true; // Show all if no machine selected
                    } else if (type === 'all') {
                        visible = true;
                    } else if (activeType === 'forklift') {
                        visible = true; // Forklift shows everything
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        Swal.fire({
                            icon: 'success',
                            title: 'Berhasil',
                            html: `${response.message}<br><strong style="font-size: 1.2rem; color: #198754;">Nilai Persentase: ${response.persentase}%</strong>`,
                            timer: 4000,
